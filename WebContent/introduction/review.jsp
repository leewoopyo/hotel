...
List<BoardDTO> boardlist = hoteldao.get_review_list(from,cnt);
...
<td style="width: 40%;">
<a href="review_oneview.jsp?id=<%=boardlist.get(i).getId()%>&viewcnt=<%=boardlist.get(i).getViewcnt()%>&userid=<%=boardlist.get(i).getUserid()%>" style="color:black;"><%=boardlist.get(i).getTitle()%></a>
...
<div class="middle_btn">
	<a class="btn btn-primary" href="review_New_content.jsp" role="button" style="background-color:#C6B3B7; border-color: #C6B3B7; position: relative; left: 85%; ">새글 작성</a>
</div>



<div class="paging">
	<a href='review.jsp?from=0' class="btn first null">FIRST</a>
<%if(from >= (10*cnt)){ %>
	<a href='review.jsp?from<%=(from-(10*cnt))-(from%(10*cnt))%>' class="btn prev null">PREV</a> 
<%}else {%>
...
<%for(int i = 1 + pagebar; i <=10 + pagebar-(10-Math.ceil((double)maxcount%(10*cnt)/cnt)); i++){ %>
	<a href='review.jsp?from=<%=(i-1)*cnt%>' ><%=i%></a>
<%}%>
<%}else{%>
...
<%if(from < maxcount-(maxcount%(10*cnt))){ %>
	<a href='review.jsp?from=<%=(from+(10*cnt))-(from%(10*cnt))%>' class="btn next">NEXT</a> 
<%}else{%>
	<a class="btn next">NEXT</a>
<%}%>
	<a class="btn last" href='review.jsp?from=<%=(int)(maxcount/cnt)*10 %>'>LAST</a>
</div>
